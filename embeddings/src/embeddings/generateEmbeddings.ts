...
const chunkFiles = async () => {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      if (fileStats.isFile()) {
        ...
        if (file.endsWith('.docx')) {
          ...
          console.log(
            'DOCX parsing: ',
            result.messages.length ? result.messages : 'No errors'
          )
  ...
  fs.writeFileSync(
    '../../../chunkedTextForEmbeddings.json',
    JSON.stringify(contentChunked, null, 2)
  )
...
export const generateEmbeddings: GenerateEmbeddings<never, string> = async (
  _args,
  context
) => {
  try {
    ...
    for (let i = 0; i < contentChunked.length; i++) {
      for (let j = 0; j < contentChunked[i].length; j++) {
        ...
        const {
          ...
          content_tokens: tokenAmount,
        } = text[j]
        const splitTitle = chunkTitle.split('-')
        const parentFileTitle = splitTitle
        ...
        if (!!existingEmbedding) {
          console.log(
            'skipping this chunk -- embedding already exists in the database'
          )
          continue
        }
